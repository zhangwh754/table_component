...
type InitData = {
  ...
  sortedKey: String;
  sort: "" | "descending" | "ascending"; // 当前的排序类型
};
...
type InitProperty = {
  ...
  sortedKeys: WechatMiniprogram.Component.FullProperty<ArrayConstructor>;
};
...
type InitMethod = {
  ...
  tipFc(): void;
  triggerThSort(e: GlobalData.WxAppletsEvent): void;
};
...
Component<InitData, InitProperty, InitMethod>({
  ...
  methods: {
      ...
      this.setData({
        showDataList: dataList.map((item, index) => {
          ...
          needReaderColums.forEach((item1) => {
            newItem[item1.key] = item1.render(
              newItem[item1.key],
              item,
              index,
              getNowPage().data
    ...
    clearScrollTag: debounce(function (
      this: WechatMiniprogram.Component.Instance<
        InitData,
        InitProperty,
        InitMethod,
    ...
    triggerThSort(e) {
      ...
      if (
        ...
        // 如果当前激活的sortKey和点击的key不同，切换为descending
      ...
      try {
        ...
        node
          .boundingClientRect((rect) => {
            this.setData({
              tableScrollViewHeight: `calc(${tableHeight} - ${
                rect.height * pageConfig.pixelRatio
              }rpx)`,
            });
          })
          .exec();
    ...
    tipFc() {
      ...
      if (!rowKey) {
        console.error(
          "table组件必须指明每一行的唯一标识的字段名，且必须为字符串，数字将会被转为字符串,for循环中的wx:key不使用该字段，用的是createShowDataList中设置的row_key字段"
        );
      }
      if (!columns) {
